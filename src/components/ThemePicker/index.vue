...
import elementJson  from 'element-plus/package.json' // element-ui version from node_modules
  ...
  setup(props, context) {
    ...
    watch(() => state.theme, async(value: string) => {
      if (value) {
        ...
        if (!state.chalk) {
          const url = `https://unpkg.com/element-plus@${elementJson['version']}/lib/theme-chalk/index.css`
